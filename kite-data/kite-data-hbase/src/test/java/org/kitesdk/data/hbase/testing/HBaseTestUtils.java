...
public class HBaseTestUtils {
    ...
    if (miniCluster == null) {
      synchronized (HBaseTestUtils.class) {
        if (miniCluster == null) {
          ...
          util.createTable(TableName.valueOf("epoch"),
              new byte[][] { Bytes.toBytes("e") });
          util.createTable(TableName.valueOf("managed_schemas"),
  ...
  public static HBaseTestingUtility setupHbase(
    ...
    for (Map.Entry<String, List<String>> entry : tableDefinitions.entrySet()) {
      ...
      if (tablesSetup.contains(tableName)) {
        util.truncateTable(TableName.valueOf(tableName));
      } else {
        ...
        util.createTable(TableName.valueOf(tableName), columnFamilyNamesBytes);
  ...
  public static SchemaManager initializeSchemaManager(
      Connection connection, String directory) throws Exception {
    SchemaManager entityManager = new DefaultSchemaManager(
        connection);
    SchemaTool schemaTool = new SchemaTool(connection.getAdmin(),
        entityManager);
